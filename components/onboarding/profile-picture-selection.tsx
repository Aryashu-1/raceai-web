...
interface ProfilePictureSelectionProps {
  onComplete: (profileData: { profilePicture: string; profileType: "upload" | "avatar"; selectedAvatar?: any }) => void
  onSkip: () => void
  userInterests: string[]
}

const ProfilePictureSelection: React.FC<ProfilePictureSelectionProps> = ({ onComplete, onSkip, userInterests }) => {
  const [selectedAvatar, setSelectedAvatar] = useState(null)
  const [uploadedImage, setUploadedImage] = useState(null)
  const [selectionType, setSelectionType] = useState<"avatar" | "upload" | null>(null)
  ...
  const hardcodedAvatars = [
    ...
    },
    {
      ...
      description: "Brilliant cosmologist who made groundbreaking discoveries about black holes",
    ...
    },
    {
      ...
      description: "Entrepreneur advancing space exploration and sustainable technology",
  ...
  const handleAvatarSelect = (avatar) => {
    ...
    setAriaMessage(`Great choice! ${avatar.name} is an incredible inspiration. ${avatar.description}`)
  }
  ...
  const handleImageUpload = (event) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        setAriaMessage("Perfect! Your own photo adds a personal touch to your research profile.")
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-8 relative overflow-hidden">
      <div className="absolute inset-0 opacity-10">
        ...
        <div className="absolute top-1/4 left-1/3 animate-float-particle" style={{ animationDelay: "3s" }}>
          <svg className="w-16 h-16 text-blue-400/20 drop-shadow-lg" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
          </svg>
        </div>
        <div className="absolute top-1/2 right-1/4 animate-float-particle" style={{ animationDelay: "5s" }}>
          <svg className="w-12 h-12 text-blue-300/15 drop-shadow-lg" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className="max-w-4xl w-full relative z-10">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-foreground mb-4">Choose your research persona</h2>
        ...
        <div className="mb-8">
          <div className="backdrop-blur-sm bg-card/80 border border-border/50 rounded-xl p-6">
            <h3 className="text-xl font-semibold text-foreground mb-4 text-center">Upload Your Photo</h3>
        ...
        {isLoading ? (
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground mt-2">Finding researchers in your field...</p>
          </div>
        ) : (
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {hardcodedAvatars.map((avatar, index) => (
                ...
                >
                  ...
                  <h4 className="text-foreground font-medium text-sm mb-1">{avatar.name}</h4>
                  <p className="text-muted-foreground text-xs">{avatar.field}</p>
                </div>
              ))}
            </div>
          </div>
