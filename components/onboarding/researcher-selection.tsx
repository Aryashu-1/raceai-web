...
const fetchResearchersByInterests = async (interests: string[]): Promise<Researcher[]> => {
  try {
    ...
    "Artificial Intelligence": [
      {
        ...
        description: "Pioneer in computer vision and AI ethics, co-director of Stanford HAI",
      ...
      },
      {
        ...
        description: "Turing Award winner, pioneer of deep learning and neural networks",
    ...
    "Machine Learning": [
      {
        ...
        description: "Co-founder of Coursera, former head of Baidu AI and Google Brain",
    ...
    "Data Science": [
      {
        ...
        description: "Author of 'Weapons of Math Destruction', expert in algorithmic bias",
    ...
    Neuroscience: [
      {
        ...
        description: "Leading researcher in emotion, decision-making, and consciousness",
...
export default function ResearcherSelection({ interests, onNext, onBack }: ResearcherSelectionProps) {
  ...
  useEffect(() => {
    const loadResearchers = async () => {
      ...
      try {
        ...
        setAriaMessage(
          `Based on your interests in ${interests.join(", ")}, here are some influential researchers you might find inspiring. Choose one that resonates with you, or skip if you prefer!`,
        )
      } catch (err) {
        setError("Failed to load researchers. Showing fallback options.")
        setResearchers(getFallbackResearchers(interests))
        setAriaMessage("I had trouble finding researchers, but here are some great options based on your interests!")
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleResearcherSelect = (researcher: Researcher) => {
    ...
    setAriaMessage(`Great choice! ${researcher.name} is known for ${researcher.description.toLowerCase()}`)
  }
  ...
  const handleSkip = () => {
    setAriaMessage("No problem! You can always explore researchers later in your dashboard.")
    onNext(null)
  }
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center mb-6">
            <AriaAssistant state={loading ? "thinking" : "speaking"} message={ariaMessage} size="md" />
          </div>
          <h1 className="text-3xl font-bold text-foreground mb-2">Top Researchers You'd Like to Follow</h1>
          <p className="text-muted-foreground">
            Select a researcher whose work inspires you, or skip this step to continue.
          </p>
        </div>
        ...
        {loading && (
          <div className="flex items-center justify-center py-12">
            ...
            <span className="ml-2 text-muted-foreground">Finding researchers in your field...</span>
          </div>
        )}
            ...
            {researchers.map((researcher) => (
              <Card
                ...
                className={`cursor-pointer transition-all duration-200 hover:shadow-lg hover:scale-105 ${
                  selectedResearcher?.id === researcher.id ? "ring-2 ring-primary bg-primary/5" : "hover:bg-accent/50"
                }`}
                ...
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-sm mb-1 truncate">{researcher.name}</h3>
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs">{researcher.citations}</span>
                        </div>
                      </div>
                      <p className="text-xs text-muted-foreground mb-2 line-clamp-2">{researcher.description}</p>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3">
            <Button variant="outline" onClick={handleSkip} className="px-6 bg-transparent">
              Skip
            </Button>
            <Button onClick={handleContinue} disabled={!selectedResearcher && !loading} className="px-8">
